...
public class SecureDirectory extends HttpServlet{
  private Perl5Util util = new Perl5Util();
	...
	public void init() throws ServletException{
	    ...
	    secureDirPath=getServletContext().getRealPath("/WEB-INF/secure");
			strongProtection=new Boolean(getInitParameter("strongProtection")).booleanValue(); 
  ...
  public long getLastModified(HttpServletRequest request){
		if(new Date().getTime()>0)return new Date().getTime(); //comment this line if you want allow browser to check when resource was last modified 
		String userID=(String)request.getSession().getAttribute("org.gridlab.gridsphere.portlet.User");
		if(userID==null || userID.equals("")){
		  if(DEBUG)
